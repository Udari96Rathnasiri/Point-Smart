...
public class PlaceOrderFormController {
    ...
    public void initialize() throws IOException {
        String[] tableCols = {"code", "description", "qty", "unitPrice", "total", "btnDelete"};
        for (int i = 0; i < tableCols.length; i++) {
            tblOrderDetails.getColumns().get(i).setCellValueFactory(new PropertyValueFactory<>(tableCols[i]));
        }
    ...
    public void newOrder() throws IOException {
        for (TextField txt : new TextField[]{txtDescription,txtCustomerName,txtQty,txtQtyOnHand,txtUnitPrice}){
            txt.clear();
            txt.setDisable(true);
        }
        ...
        try {
            ...
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR,"Failed to established the database connection, try later!").show();
            navigateToHome(null);
        }
